...
namespace YourSharingEconomyApp
{
	...
	public class RequestModel
	{
		...
		private string m_signatureCustomer;
		private string m_signatureProvider;

		private string m_transactionIdBitcoin;
		...
		public string TransactionIdBitcoin
		{
			...
			set { m_transactionIdBitcoin = value; }
		}
		...
		public RequestModel(long _id,
							...
							string _signatureProvider,
							string _transactionIdBitcoin
						)
		{
			m_id = _id;
			...
			m_signatureCustomer = _signatureCustomer;
			m_signatureProvider = _signatureProvider;

			m_transactionIdBitcoin = _transactionIdBitcoin;
		}
		...
		public void Copy(RequestModel _request, bool _images)
		{
			...
			m_signatureCustomer = _request.SignatureCustomer;
			m_signatureProvider = _request.SignatureProvider;
		}
		...
		* GetBinaryData
		*/
		private byte[] GetBinaryData(bool _isCustomer)
		{
			List<byte[]> binaryData = new List<byte[]>();
			
			binaryData.Add(BitConverter.GetBytes(m_id));
			binaryData.Add(BitConverter.GetBytes(m_customer));
			binaryData.Add(BitConverter.GetBytes(m_provider));
			binaryData.Add(Encoding.UTF8.GetBytes(m_title));
			binaryData.Add(Encoding.UTF8.GetBytes(m_description));			
			binaryData.Add(Encoding.UTF8.GetBytes(m_village));
			binaryData.Add(BitConverter.GetBytes(m_price));
			binaryData.Add(Encoding.UTF8.GetBytes(m_currency));
			binaryData.Add(BitConverter.GetBytes(m_deadline));
			binaryData.Add(BitConverter.GetBytes(m_deliverydate));
			binaryData.Add(Encoding.UTF8.GetBytes(m_transactionIdBitcoin));
			if (_isCustomer)
			{
				binaryData.Add(Encoding.UTF8.GetBytes(m_feedbackcustomer));
				binaryData.Add(BitConverter.GetBytes(m_scorecustomer));
			}
			else
			{
				binaryData.Add(Encoding.UTF8.GetBytes(m_feedbackprovider));
			...
			{
				counter += binaryData[i].Length;
			}

			byte[] dataRequest = new byte[counter];
			int position = 0;
			for (int i = 0; i < binaryData.Count; i++)
			{
				Array.Copy(binaryData[i], 0, dataRequest, position, binaryData[i].Length);
				position += binaryData[i].Length;
			}
