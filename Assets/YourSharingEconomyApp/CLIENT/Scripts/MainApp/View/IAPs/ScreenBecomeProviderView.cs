...
public class ScreenBecomeProviderView : ScreenBaseView, IBasicScreenView
{
    ...
    public void Initialize(params object[] _list)
    {
        ...
        m_container.Find("Title").GetComponent<Text>().text = LanguageController.Instance.GetText("message.iap.become.provider");

        m_container.Find("Button_Rent1").GetComponent<Button>().onClick.AddListener(OnRent1Pressed);
        m_container.Find("Button_Rent1/Text").GetComponent<Text>().text = LanguageController.Instance.GetText("message.iap.rent.provider.1");

        m_container.Find("Button_Rent2").GetComponent<Button>().onClick.AddListener(OnRent2Pressed);
        m_container.Find("Button_Rent2/Text").GetComponent<Text>().text = LanguageController.Instance.GetText("message.iap.rent.provider.2");

        m_container.Find("Button_Rent3").GetComponent<Button>().onClick.AddListener(OnRent3Pressed);
        m_container.Find("Button_Rent3/Text").GetComponent<Text>().text = LanguageController.Instance.GetText("message.iap.rent.provider.3");

        m_container.Find("Button_Rent4").GetComponent<Button>().onClick.AddListener(OnRent4Pressed);
        m_container.Find("Button_Rent4/Text").GetComponent<Text>().text = LanguageController.Instance.GetText("message.iap.rent.provider.4");
    ...
    private void OnBasicEvent(string _nameEvent, params object[] _list)
    {
        if (_nameEvent == UsersController.EVENT_USER_IAP_CALL_PURCHASE_RENT_PROVIDER)
        {
            if (!(bool)_list[0])
            {
                ...
                string description = LanguageController.Instance.GetText("message.iap.failure.become.provider");
        ...
        if (_nameEvent == UsersController.EVENT_USER_IAP_RESULT_PURCHASE_RENT_PROVIDER)
        {
            if ((bool)_list[0])
            {
                ...
                string description = LanguageController.Instance.GetText("message.iap.congratulations.provider");
